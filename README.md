...
- [x] **Clientless Remote Desktop Gateway (Apache Guacamole & Multi-Protocol Auth)**: Engineered a production-grade HTML5 clientless remote desktop gateway (`guacamole.lambertlab.us`) on Kubernetes backed by PostgreSQL with persistent block storage on Longhorn (`longhorn-retain`). Key architecture achievements include:
  - **Active Directory LDAPS Integration (`636/TCP`)**: Configured secure directory authentication against the virtualized Windows Server domain controller (`dc01-ad.vms:636`), dynamically injecting the internal Active Directory Root CA certificate (`lambertlab-DC01-CA`) into Java's truststore (`/etc/ssl/certs/java/custom-cacerts`) via an automated Alpine initContainer (`eclipse-temurin:21-jre-alpine`).
  - **Dual-Protocol Identity & SSO Federation**: Configured enterprise OpenID Connect (OIDC) single sign-on backed by Microsoft Entra ID (Azure AD) with extension priority chaining (`EXTENSION_PRIORITY: "*,openid"`), enabling seamless corporate SSO and auto-provisioning while preserving database/LDAP authentication and emergency `guacadmin` local fallback.
  - **Edge Ingress & Real-IP Reverse Proxying**: Exposed through Traefik ingress with automated HTTP-to-HTTPS redirection and admin-only network middleware; configured Tomcat `RemoteIpValve` with `allowedIpsRegex: ".*"` to prevent proxy lockouts and preserve client IPs.
  - **Internal Hairpin & DNAT Resolution**: Resolved Tailscale public routing isolation for internal virtual machines via OPNsense LAN port forwarding (`100.64.0.0/10` to Traefik ClusterIP `10.43.204.125:443`), enabling VMs without Tailscale clients to securely access the gateway.
  - **Workstation Workload Isolation & QoS Tuning**: Tuned container CPU and memory requests/limits (`guacd`: 2000m CPU limit; `guacamole`: 1000m CPU / 1024Mi limit) to guarantee smooth 60fps RDP rendering while preserving Intel Xeon CPU threads and NVIDIA RTX A4500 compute for host workstation tasks and Docker ELK stack logging.
