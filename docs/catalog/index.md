...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **ArgoCD HA** | Infrastructure | `argocd` | ArgoCD Helm (Wave 2) | `argocd.lambertlab.us` | Longhorn Distributed Block |
...
| **OPNsense Firewall** | Virtualization | `vms` (KubeVirt) | ArgoCD Manifest (Wave 3) | `opnsense.lambertlab.us` | Longhorn Block |
...
| **Jellyfin** | Media | `media` | ArgoCD Multi-Source (Wave 3) | `jellyfin.lambertlab.us` | 14TB TerraMaster NFS Pool |
| **Sonarr** | Media | `media` | ArgoCD Manifest (Wave 3) | `sonarr.lambertlab.us` | 14TB TerraMaster NFS Pool |
| **Radarr** | Media | `media` | ArgoCD Manifest (Wave 3) | `radarr.lambertlab.us` | 14TB TerraMaster NFS Pool |
| **Prowlarr** | Media | `media` | ArgoCD Manifest (Wave 3) | `prowlarr.lambertlab.us` | 14TB TerraMaster NFS Pool |
| **Tdarr (Transcoder)** | Media | `workstation` (Docker) | Terraform Docker | `tdarr.lambertlab.us` | 14TB TerraMaster NFS Pool |
| **Palworld Dedicated** | Gaming | `gaming` | ArgoCD Manifest (Wave 3) | Port 8211 UDP (`opti74`) | Longhorn Retained Volume |
| **HashiCorp Vault** | Security | `security` | ArgoCD Helm (Wave 2) | `vault.lambertlab.us` | 10 GB Longhorn Retained |
...
| **Homarr Dashboard** | Observability | `observability` | ArgoCD Manifest (Wave 3) | `homarr.lambertlab.us` | Longhorn Distributed Block |
| **Ntfy Notifications** | Observability | `observability` | ArgoCD Manifest (Wave 3) | `ntfy.lambertlab.us` | Longhorn Retained Volume |
| **Beszel Hub & Agents** | Observability | `lenovo` (Hub) / Nodes | Terraform Docker | `beszel.lambertlab.us` | Local Docker Volume |
| **Pi-hole** | Security | `external` / Docker | EndpointSlice & Ingress | `pihole.lambertlab.us` | Local Volume |
| **Portainer** | Management | `optiplex` (Docker) | Terraform Docker | `portainer.lambertlab.us` | Local Docker Volume |
| **Cockpit** | Management | `lenovo` / Node | EndpointSlice & Ingress | `cockpit.lambertlab.us` | Host System |
...
| **Redis & RedisInsight** | Caching | `workstation` (Docker) | Terraform Docker | `redisinsight.lambertlab.us` | Local Docker Volume |
| **RetroArch Web** | Gaming | `workstation` (Docker) | Terraform Docker | `retroarch.lambertlab.us` | Local Docker Volume |
