...
resource "nginxproxymanager_proxy_host" "vault_proxy" {
...
# NAS proxy host: Routes external/internal traffic from nas.homelab.domain sub-domain to the NAS web interface
resource "nginxproxymanager_proxy_host" "nas_proxy" {
  domain_names            = ["nas.${var.homlab_domain}"]
  access_list_id          = var.access_list_id
  forward_scheme          = "http"
  forward_host            = var.nas_ip
  forward_port            = 8181
  caching_enabled         = true
  allow_websocket_upgrade = true
  block_exploits          = true
